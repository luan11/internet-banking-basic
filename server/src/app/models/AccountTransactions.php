		...
		if(!empty($confirmSender) && $confirmSender['account'] !== $accountReceiver){
			if($transact->transactValue <= floatval($confirmSender['balance'])){
				...
				if(!empty($confirmReceiver)){
					...
					try{
						\src\app\services\Database::doTransact($transact->transactConfirmId, $senderNewBalance);
						try{
							\src\app\services\Database::saveTransactHistory('Envio p/conta ['.$accountReceiver.']', -$transact->transactValue, $transact->transactIp, $transact->transactConfirmId);
							try{
								\src\app\services\Database::doTransact($confirmReceiver->id_ibbUsers, $receiverNewBalance);
								try{
									\src\app\services\Database::saveTransactHistory('Recebimento da conta ['.$confirmSender['account'].']', +$transact->transactValue, $transact->transactIp, $confirmReceiver->id_ibbUsers);
									$transact->setSuccess("Transferência realizada com sucesso!");
								}catch(\Exception $e){
									$transact->setError($e->getMessage()." <b>(CÓDIGO DO ERRO: #".$e->getCode().")</b>");
								}
							}catch(\Exception $e){
								$transact->setError($e->getMessage()." <b>(CÓDIGO DO ERRO: #".$e->getCode().")</b>");
							}
		...
		if(!empty($confirmRequester)){
			if($transact->transactValue <= floatval($confirmRequester['balance'])){
				...
				try{
					\src\app\services\Database::doTransact($transact->transactConfirmId, $senderNewBalance);
					try{
						\src\app\services\Database::saveTransactHistory('Retirada', -$transact->transactValue, $transact->transactIp, $transact->transactConfirmId);
	...
	public function deposit(AccountTransactions $transact){
		...
		if(!empty($confirmRequester)){
			...
			try{
				\src\app\services\Database::doTransact($transact->transactConfirmId, $senderNewBalance);
				try{
					\src\app\services\Database::saveTransactHistory('Depósito', +$transact->transactValue, $transact->transactIp, $transact->transactConfirmId);
